...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <a href="<?= url('/shipments') ?>" class="text-xs text-emerald-700 font-bold hover:underline mb-1 inline-block">
        ...
        <div class="flex gap-3">
            <a href="<?= url('/shipments/label?id=' . $shipment['id']) ?>" target="_blank" class="px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-sm shadow-md flex items-center gap-2">
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-3">
            ...
            <div>
                ...
                <p class="text-xs text-emerald-700 font-bold mt-0.5"><i class="ri-phone-line"></i> <?= e($shipment['sender_phone']) ?></p>
        ...
        <div class="space-y-3">
            ...
            <div>
                ...
                <p class="text-xs text-emerald-700 font-bold mt-0.5"><i class="ri-phone-line"></i> <?= e($shipment['receiver_phone']) ?></p>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 space-y-6">
        <div class="flex flex-col sm:flex-row items-center justify-between border-b border-gray-100 pb-6">
            <div>
                ...
                <p class="text-xs text-gray-400">Barcode ini digunakan untuk scan kamera HP oleh petugas / kurir BDL.</p>
            </div>
                ...
                <div class="relative border-l-2 border-emerald-100 ml-4 space-y-6">
                    <?php foreach (array_reverse($checkpoints) as $index => $cp): ?>
                        <div class="relative pl-8">
                            <div class="absolute -left-[17px] top-0 w-8 h-8 rounded-full flex items-center justify-center text-white <?= $index === 0 ? 'bg-emerald-600 ring-4 ring-emerald-100 shadow-md' : 'bg-gray-300' ?>">
                                ...
                                <div class="flex items-center justify-between text-xs text-gray-400 mb-1">
                                    ...
                                    <div title="Petugas: <?= e($cp['user_name'] ?? 'System') ?>">
                                        <?php if (!empty($cp['user_avatar'])): ?>
                                            <img src="<?= url('/' . $cp['user_avatar']) ?>" alt="Petugas" class="w-6 h-6 rounded-full shadow-sm object-cover">
                                        <?php else: ?>
                                            <img src="https://ui-avatars.com/api/?name=<?= urlencode($cp['user_name'] ?? 'System') ?>&background=random&color=fff&size=28" alt="Petugas" class="w-6 h-6 rounded-full shadow-sm">
                                        <?php endif; ?>
                                    </div>
                                </div>
